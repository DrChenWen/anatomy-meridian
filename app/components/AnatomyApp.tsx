...
export function AnatomyApp({ locale, dictionary }: { locale: LocaleConfig; dictionary: Dictionary }) {
  ...
  // Compare with a paired or contrasting meridian
  const reference = organById[organId === "heart" ? "small-intestine" : "heart"];
    ...
    <main className="app-shell">
      ...
      <div className="workspace">
        ...
        <aside className="info-panel" ref={contentRef}>
          ...
          <dl className="key-facts">
            <div data-reveal><dt><span>◇</span> {t.info.yinYang ?? t.info.size}</dt><dd><Measure>{organ.yinYang ?? organ.size ?? "—"}</Measure></dd></div>
            <div data-reveal><dt><span>♙</span> {t.info.element ?? t.info.weight}</dt><dd><Measure>{organ.element ?? organ.weight ?? "—"}</Measure></dd></div>
            <div data-reveal><dt><span>⌁</span> {t.info.peakTime ?? t.info.daily}</dt><dd><Measure>{organ.peakTime ?? organ.dailyFact ?? "—"}</Measure></dd></div>
            <div data-reveal><dt><span>⌖</span> {t.info.location}</dt><dd><Measure>{organ.location}</Measure></dd></div>
            <div data-reveal><dt><span>❋</span> {t.info.keyPoint ?? t.info.bloodSupply}</dt><dd><Measure>{organ.keyPoint ?? organ.bloodSupply ?? "—"}</Measure></dd></div>
      ...
      {compare && (
        <section className="compare-strip" aria-label={t.compare.title}>
          ...
          <dl><div><dt>{t.compare.primaryRole}</dt><dd><Measure>{organ.function}</Measure></dd></div><div><dt>{t.compare.scale}</dt><dd><Measure>{organ.location}</Measure></dd></div></dl>
        ...
        {type === "quiz" ? (
          ...
          <>
            ...
            <dl className="modal-facts">
              ...
              <div><dt>{t.info.keyPoint ?? t.info.bloodSupply}</dt><dd><Measure>{organ.keyPoint ?? organ.bloodSupply ?? "—"}</Measure></dd></div>
            </dl>
